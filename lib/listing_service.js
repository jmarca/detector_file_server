...
function listing_service( options ){
    ...
    return function listing_service(req,res,next){
        ...
        glob(pattern,{cwd:searchpath},function(er,files){
	    if(!files){
	        return next();
	    }
	    if(files.length){
...
function rfiles(app,options) {
    ...
    app.get('/vdsdata/*RData'
           ,express.static(pems_assets_dir)
           );

    app.get('/wimdata/*RData'
           ,express.static(wim_assets_dir)
           );
    return app;
}
...
function qfiles(app,options) {
    ...
    app.get('/vdsdata/:district/:freeway?'
           ,listing_service({'root': pems_assets_dir})
           );
    app.get('/wimdata/:year?/:site?'
           ,listing_service({'root': wim_assets_dir
                            ,'params':['year','site']
                            })
           );
    return app;
